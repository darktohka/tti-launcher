...
QUrl channelUrl(const QString &downloadBase, const QString &distribution,
                const QString &path) {
  ...
  const QString url =
      base + QLatin1Char('/') + distribution + QLatin1Char('/') + path;
  return QUrl(url);
}
...
QString Updater::installDir() { return QDir::currentPath(); }
...
void Updater::login(const QString &username, const QString &password,
  ...
  connect(m_loginReply, &QNetworkReply::finished, this,
          &Updater::onLoginReplyFinished);
}
...
void Updater::fetchManifest() {
  ...
  const QUrl url = channelUrl(downloadBase, launcher::kDistribution,
  ...
  connect(m_manifestReply, &QNetworkReply::finished, this,
          &Updater::onManifestReplyFinished);
}
...
void Updater::parseTree(const QJsonObject &object, const QString &parentPath) {
  ...
  if (!containsPlatform(object.value(QStringLiteral("platforms")),
                        launcher::kPlatform)) {
    return;
  }

  const QJsonArray directories =
      object.value(QStringLiteral("directories")).toArray();
  ...
  for (const QJsonValue &value : directories) {
    ...
    const QString path =
        parentPath.isEmpty() ? name : parentPath + QStringLiteral("/") + name;
    parseTree(dir, path);
  }
  ...
  for (const QJsonValue &value : filesArray) {
    ...
    if (!containsPlatform(file.value(QStringLiteral("platforms")),
                          launcher::kPlatform)) {
      continue;
    }
...
QString Updater::formatProgress(qint64 bytesRead, qint64 bytesTotal, int index,
                                int total) {
  ...
  const double speed =
      double(bytesRead - m_lastBytesRead) * 1000.0 / double(dt);
  ...
  return launcher::kUpdatingFile.arg(index + 1)
      .arg(total)
...
void Updater::setStatus(const QString &status) { emit statusChanged(status); }